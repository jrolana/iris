...
\usepackage{pdfpages}
\usepackage{caption}
% left-align table captions; keep them at the top
\captionsetup[table]{justification=raggedright,singlelinecheck=false,position=top}

\usepackage{float}    % for [H]
\usepackage[natbibapa]{apacite} % gives you \citet / \citep in APA style
\usepackage[hidelinks]{hyperref} % load after apacite
\usepackage{url}
\usepackage{etoolbox}
\robustify\url  
...
\usepackage{tabularx}
\usepackage{subcaption} % for subfigures (a), (b), ...
\usepackage[left]{lineno}   % numbers on the left (recommended)
\usepackage{hyperref}
\usepackage{url}
...
% Prevent document line numbers from overlapping code listing line numbers.
\AtBeginEnvironment{lstlisting}{\par\nolinenumbers}
\AtEndEnvironment{lstlisting}{\par\linenumbers}
