...
PDF_PATH = OUTPUT_DIR / "techgen-source-deposit-first-last-10-pages.pdf"
MANIFEST_PATH = OUTPUT_DIR / "techgen-source-deposit-manifest.txt"

PROGRAM_TITLE = "IRIS - TechGen Use Case Source Code"
VERSION_LABEL = "Source snapshot: 2026-06-10"
LINES_PER_PAGE = 72
MAX_TEXT_CHARS = 118

SOURCE_FILES = [
    "src/app/techgen/layout.tsx",
    "src/app/techgen/page.tsx",
    "src/app/techgen/application-registry/page.tsx",
    "src/app/techgen/application-guide/page.tsx",
    "src/app/techgen/application-document/page.tsx",
    "src/app/techgen/notifications/page.tsx",
    "src/app/techgen/new-application/page.tsx",
    "src/app/techgen/start-application/page.tsx",
    "src/app/techgen/start-application/StartApplicationPageClient.tsx",
    "src/app/techgen/view-application/page.tsx",
    "src/app/techgen/view-application/TechgenViewApplicationPageClient.tsx",
    "src/components/techgen/MetricsTechgen.tsx",
    "src/components/techgen/StatusUpdatesPanel.tsx",
    "src/components/application/ApplicationView.tsx",
    "src/components/application/InformationPanel.tsx",
    "src/components/application/ViewInventorsTechGen.tsx",
    "src/components/application/RequirementsPanel.tsx",
    "src/components/application/StatusHistoryPanel.tsx",
    "src/components/common/ApplicationsTable.tsx",
    "src/components/common/ApplicationGuidePage.tsx",
    "src/components/common/ApplicationDocuments.tsx",
    "src/components/common/ViewAllNotifications.tsx",
    ...
    "src/lib/roles.ts",
    "src/lib/constants/new-application.ts",
]
    ...
    for source_file in SOURCE_FILES:
        ...
        for line_number, line in enumerate(raw_lines, start=1):
            ...
            for chunk in chunks[1:]:
                listing.append(continuation_prefix + chunk)
...
def write_manifest(manifest_rows: list[tuple[str, int]], total_pages: int) -> None:
    total_lines = sum(line_count for _, line_count in manifest_rows)
    lines = [
        PROGRAM_TITLE,
        VERSION_LABEL,
        "",
        "Purpose: first and last 10 rendered source pages for copyright deposit.",
...
def main() -> None:
    ...
    first_pages = [(page, index + 1, "FIRST 10 SOURCE PAGES") for index, page in enumerate(pages[:10])]
    last_start = max(total_pages - 10, 0)
    last_pages = [
        (page, last_start + index + 1, "LAST 10 SOURCE PAGES")
        for index, page in enumerate(pages[last_start:])
    ]

    write_pdf(first_pages + last_pages, total_pages)
    write_manifest(manifest_rows, total_pages)

    print(f"Wrote {PDF_PATH}")
    print(f"Wrote {MANIFEST_PATH}")
    print(f"Total rendered source pages: {total_pages}")
    print(f"PDF pages included: {len(first_pages) + len(last_pages)}")
