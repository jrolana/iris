...
export async function GET(request: Request) {
  ...
  const supabase = createServerClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        setAll: (cookieList) => cookieList.forEach(({ name, value, options }) =>
  ...
  // Handle OAuth code
  if (!code) return redirectWithError('No authentication code found. Please try signing in again.')

  const { data: { session }, error: sessionError } = await supabase.auth.exchangeCodeForSession(code)
  if (sessionError || !session?.user) return redirectWithError('Authentication failed. Please try signing in again.')
