...
export const UserSchemaBase = z.object({
  ...
  college_code: optionalText.pipe(z.string().max(20).optional()),
  other_college_name: optionalText,
  external_institution: optionalText,
});

export const UserSchema = UserSchemaBase.superRefine((data, ctx) => {
  ...
  if (
    ...
    !data.external_institution
  ) {
    const isUpEmail = data.email.endsWith("up.edu.ph");
    ctx.addIssue({
      code: "custom",
      message: isUpEmail ? "College is required" : "Institution is required",
      path: [isUpEmail ? "college_code" : "external_institution"],
    });
  }
});
